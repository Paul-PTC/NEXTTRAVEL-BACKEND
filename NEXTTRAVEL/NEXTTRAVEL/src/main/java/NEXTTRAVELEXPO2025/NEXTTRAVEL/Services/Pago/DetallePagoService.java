...
public class DetallePagoService {
    ...
    // ===== Listado / Búsquedas =====
    public Page<DetallePagoDTO> listar(Pageable p) { return repo.findAll(p).map(this::toDTO); }
    ...
    public Long crear(@Valid DetallePagoDTO dto) {
        Pago pago = pagoRepo.findById(dto.getIdPago())
                .orElseThrow(() -> new EntityNotFoundException("No existe Pago con id: " + dto.getIdPago()));
    ...
    public void actualizar(Long id, @Valid DetallePagoDTO dto) {
        DetallePago e = repo.findById(id)
                .orElseThrow(() -> new EntityNotFoundException("No se encontró DetallePago con id: " + id));

        if (dto.getIdPago() != null && (e.getPago() == null ||
                !dto.getIdPago().equals(e.getPago().getIdPago()))) {
            Pago pago = pagoRepo.findById(dto.getIdPago())
                    .orElseThrow(() -> new EntityNotFoundException("No existe Pago con id: " + dto.getIdPago()));
            e.setPago(pago);
        }
