...
public class VwClienteService {
    ...
    public Page<VwClienteDTO> listar(Pageable p) { return repo.findAll(p).map(this::toDTO); }

    public Page<VwClienteDTO> buscarPorNombre(String q, Pageable p) { return repo.findByNombreContainingIgnoreCase(q, p).map(this::toDTO); }
    public Page<VwClienteDTO> buscarPorCorreo(String q, Pageable p) { return repo.findByCorreoContainingIgnoreCase(q, p).map(this::toDTO); }
    public Page<VwClienteDTO> buscarPorTelefono(String q, Pageable p) { return repo.findByTelefonoContainingIgnoreCase(q, p).map(this::toDTO); }
    public Page<VwClienteDTO> buscarPorDireccion(String q, Pageable p) { return repo.findByDireccionContainingIgnoreCase(q, p).map(this::toDTO); }
