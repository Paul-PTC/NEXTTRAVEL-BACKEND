...
import NEXTTRAVELEXPO2025.NEXTTRAVEL.Services.Pago.TipoGastoService;
...
@Slf4j
@RestController
@RequestMapping("/api/tipo-gastos")
...
public class TipoGastoController {
    ...
    // ===== GET listar -> /api/tipo-gastos/tipos/listar
    @GetMapping("/tipos/listar")
    public ResponseEntity<Page<TipoGastoDTO>> listar(
    ...
    // por nombre -> /api/tipo-gastos/tipos/buscar/nombre/{q}
    @GetMapping("/tipos/buscar/nombre/{q}")
    public ResponseEntity<Page<TipoGastoDTO>> buscarPorNombre(
    ...
    // por descripción -> /api/tipo-gastos/tipos/buscar/descripcion/{q}
    @GetMapping("/tipos/buscar/descripcion/{q}")
    public ResponseEntity<Page<TipoGastoDTO>> buscarPorDescripcion(
            @PathVariable String q,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size,
            @RequestParam(defaultValue = "nombreTipo,asc") String sort
    ...
    // ===== POST crear -> /api/tipo-gastos/tipos
    @PostMapping("/tipos")
    public ResponseEntity<?> crear(@Valid @RequestBody TipoGastoDTO dto, BindingResult result) {
        ...
        try {
            ...
            log.error("Error al crear TipoGasto", e);
    ...
    // ===== PUT actualizar -> /api/tipo-gastos/tipos/{id}
    @PutMapping("/tipos/{id}")
    public ResponseEntity<?> actualizar(@PathVariable Long id,
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(Map.of("error", "TipoGasto no encontrado", "mensaje", e.getMessage()));
            ...
            log.error("Error al actualizar TipoGasto {}: {}", id, e.getMessage());
            return ResponseEntity.internalServerError()
                    .body(Map.of("error", "Error al actualizar tipo de gasto", "detalle", e.getMessage()));
        }
    }

    // ===== DELETE eliminar -> /api/tipo-gastos/tipos/{id}
    ...
    public ResponseEntity<?> eliminar(@PathVariable Long id) {
        try {
            ...
            if (!eliminado) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND)
                        .body(Map.of("error", "TipoGasto no encontrado"));
            }
            return ResponseEntity.ok(Map.of("mensaje", "Tipo de gasto eliminado correctamente"));
        } catch (Exception e) {
            log.error("Error al eliminar TipoGasto {}: {}", id, e.getMessage());
            return ResponseEntity.internalServerError()
